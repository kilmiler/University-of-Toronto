...
private:
   ...
   // You will need to add additional private functions
    TreeNode* getNewNode(DBentry* data);
    
    void insert_sup_root(TreeNode* rootPtr, int data);
    bool insert_sup_DBentry(DBentry* newEntry, TreeNode* data);
    
    Dentry* find_sup(TreeNode* nodePtr, string name);
    
    TreeNode* findLeftMax_sup(TreeNode* nodePtr, TreeNode* nodePtr_r);
    bool removal_sup(TreeNode* nodePtr, string name);
    
    void clear_sup(TreeNode* head);
    
    void countActive_sup(TreeNode* head) const;
